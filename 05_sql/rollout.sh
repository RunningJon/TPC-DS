...
for i in $(ls $PWD/*.sql); do
	...
	if [ "$EXPLAIN_ANALYZE" == "false" ]; then
		...
		echo "psql -A -q -t -P pager=off -v ON_ERROR_STOP=ON -v EXPLAIN_ANALYZE=\"EXPLAIN ANALYZE\" -f $i 2>&1 > $mylogfile"
		psql -A -q -t -P pager=off -v ON_ERROR_STOP=ON -v EXPLAIN_ANALYZE="EXPLAIN ANALYZE" -f $i 2>&1 > $mylogfile
		tuples="0"
	fi
	log $tuples
done
